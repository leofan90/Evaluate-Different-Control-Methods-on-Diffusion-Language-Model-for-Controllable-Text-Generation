...
from model.latent_classification_transformer import LatentClassificationTransformer

from diffusion.latent_classifier_trainer import LatentClassifierTrainer
...
def main(args):
    ...
    classifier = LatentClassificationTransformer(
        tx_dim = args.tx_dim,
        tx_depth = args.tx_depth,
        heads = args.tx_dim // ATTN_HEAD_DIM,
        latent_dim = config.d_model,
        num_classes = CONSTANTS.NUM_CLASSES[args.dataset_name],
        dropout = 0 if args.disable_dropout else 0.1,
        scale_shift = args.scale_shift,
    ).cuda()
    ...
    trainer.load_diffusion(args.resume_diffusion_dir)

    if args.gen_data:
        trainer.load(args.resume_classifier_dir)
        for seed in [42]:
    ...
    if args.eval:
        ...
        return
    if args.eval_test:
        print(args.resume_classifier_dir)
        trainer.load(args.resume_classifier_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--resume_diffusion_dir", type=str, default=None)
    parser.add_argument("--resume_classifier_dir", type=str, default=None)
    ...
    if args.eval or args.resume_training:
        assert args.resume_classifier_dir is not None

    if args.eval or args.resume_training or args.gen_data or args.eval_test:
        with open(os.path.join(args.resume_classifier_dir, 'args.json'), 'rt') as f:
            saved_args = json.load(f)
        args_dict = vars(args)
        if args.eval or args.gen_data or args.eval_test:
            heldout_params = {'wandb_name', 'output_dir', 'resume_classifier_dir', 'eval', 'eval_test', 'gen_data', 'ddim_sampling_eta', 'num_samples', 'sampling_timesteps'}
