...
def get_dataset(dataset_name, metadata=False, synthetic_train_path=None):
    ...
    elif dataset_name == 'humor_speech':
        data_path = '/home/ckfan/controlled_text_generation/datasets/humor_speech'
        dataset = load_dataset(data_path)
        train_ds = dataset['train']
        train_val_ds = train_ds.train_test_split(test_size=100, seed=42)
        train_val_ds['valid'] = train_val_ds['test']
        train_val_ds['test'] = dataset['test']
        dataset = process_humor_speech_dataset(train_val_ds)
    else:
...
def process_ag_news_dataset(dataset):
    ...
    dataset = dataset.map(process_ag_news_text, load_from_cache_file=False, remove_columns=['title', 'description']).cast_column('label', Value(dtype='int64', id=None))
    # dataset = dataset.map(process_ag_news_text, load_from_cache_file=False)
    return dataset
    
def process_humor_speech_dataset(dataset):
    def process_humor_speech_text(example):
        return {'text': PreTrainedTokenizerBase.clean_up_tokenization(example["text"].strip()), 'label':example['label']}
        # return {'text': PreTrainedTokenizerBase.clean_up_tokenization(example["description"].strip()), 'label':example['label']-1}
    dataset = dataset.map(process_humor_speech_text, load_from_cache_file=False, remove_columns=['text', 'idx']).cast_column('label', Value(dtype='int64', id=None))
