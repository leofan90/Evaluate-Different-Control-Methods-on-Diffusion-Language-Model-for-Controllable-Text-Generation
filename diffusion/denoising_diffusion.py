...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def naive_diffusion_lm_sample(self, shape, lengths, class_id=None, classifier=None, clamp=False, vocab_emb=None):
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            ...
            if clamp and vocab_emb is not None:
                x_start = clamp_to_nearest_embedding(x_start, vocab_emb)
    ...
    @torch.no_grad()
    def ddim_sample(self, shape, lengths, class_id=None, classifier=None, clamp=False, vocab_emb=None):
        batch, device, total_timesteps, sampling_timesteps, eta, objective = shape[0], self.betas.device, self.num_timesteps, self.sampling_timesteps, self.ddim_sampling_eta, self.objective
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            ...
            if class_id is not None and classifier is not None:
                # control
                latent_param = torch.nn.Parameter(latent)

                label = torch.fill((batch,), class_id, device=device, dtype=torch.long)
                with torch.enable_grad():
                    optimizer = torch.optim.Adagrad([latent_param], lr=0.1)
                    for i in range(3):
                        optimizer.zero_grad()
                        logits = classifier(latent_param, mask, time_next)
                        _, loss = classifier.compute_metrics(logits, label)
                        loss.backward()
                        optimizer.step()
                
                latent = latent_param.detach()

        return (latent, mask)

    @torch.no_grad()
    def sample(self, batch_size, length, class_id=None, classifier=None, clamp=False, vocab_emb=None):
        ...
        sample_fn = self.naive_diffusion_lm_sample
        return sample_fn((batch_size, max_seq_len, latent_dim), length, class_id, classifier, clamp, vocab_emb)
...
class Trainer(object):
    ...
    @torch.no_grad()
    def sample(self, num_samples=None, class_id=None, seed=42, test=False, clamp=False):
        ...
        if clamp:
            vocab_emb = self.bart_model.model.shared.weight
        else:
            vocab_emb = None
        ...
        while min([len(all_texts_lists[ele]) for ele in all_texts_lists]) < num_samples:
            ...
            model_outputs = list(map(lambda n: tuple(x.to('cpu') for x in self.ema.ema_model.sample(batch_size=n, length=self.length_categorical.sample((n,)), class_id=get_class_id(n), clamp=clamp, vocab_emb=vocab_emb)), batches))
                        ...
                        if self.diffusion.diffusion_model.class_conditional:
                            for class_id in range(self.diffusion.diffusion_model.num_classes):
                                if self.args.dataset_name == 'ag_news':
                                    ...
                                    num_samples = 500
                                elif self.args.dataset_name == 'humor_speech':
                                    num_samples = 100
